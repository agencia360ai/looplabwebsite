  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <h2 className="text-[clamp(2rem,5vw,4rem)] font-bold leading-[1.05] tracking-[-0.04em] uppercase mb-6 max-w-3xl">
          Six Self-RPG Apps.{" "}
...
function AppCard({ app, index }: { app: App; index: number }) {
  ...
  return (
    <div className="group relative aspect-[3/4] bg-white/[0.03] border border-white/10 rounded-lg overflow-hidden hover:border-primary/40 transition-colors">
      <span className="absolute top-5 right-5 text-[5rem] leading-none font-bold text-white/[0.04] tracking-tight select-none">
        {num}
      </span>
      {app.mascot && (
        <img
      ...
      <div className="absolute inset-x-0 bottom-0 p-5 bg-gradient-to-t from-black via-black/85 to-transparent pt-16">
        <div className="flex items-center gap-2 mb-3">
          <span
            className={`size-1.5 rounded-full ${
              app.featured ? "bg-primary" : "bg-foreground/30"
            }`}
          />
        ...
        <h3 className="text-foreground text-lg md:text-xl font-semibold tracking-tight mb-1">
          {app.title}
        </h3>
        <p className="text-foreground/60 text-sm font-light">{app.tagline}</p>
      </div>
    </div>
  );
}
